                            ...
                            <tbody>
                                <?php
                                    ...
                                    while($myrow = mysqli_fetch_assoc($myresult)){
                                        ...
                                        $itemsql = "SELECT * FROM `book` WHERE bookId = $bookId";
                                        $itemresult = mysqli_query($conn, $itemsql);
                                        $itemrow = mysqli_fetch_assoc($itemresult);
                                        $bookName = $itemrow['bookName'];
                                        $writer = $itemrow['writer'];
                                        $bookPrice = $itemrow['bookPrice'];
                                        $bookDesc = $itemrow['bookDesc'];
                                        $bookCategorieId = $itemrow['bookCategorieId'];

                                        echo '<tr>
                                                <th scope="row">
                                                    <div class="p-2">
                                                    <img src="img/book-'.$bookId. '.jpg" alt="" width="70" class="img-fluid rounded shadow-sm">
                                                    <div class="ml-3 d-inline-block align-middle">
                                                        <h5 class="mb-0"> <a href="#" class="text-dark d-inline-block align-middle">'.$bookName. '</a></h5><span class="text-muted font-weight-normal font-italic d-block">Rs. ' .$bookPrice. '/-</span>
                                                    </div>
                                                    </div>
                                                </th>
