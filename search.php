...
<body>
        ...
        <?php 
            ...
            while($row = mysqli_fetch_assoc($result)){
                ...
                echo '<div class="col-xs-3 col-sm-3 col-md-3">
                    ...
                    <a href="viewbookList.php?catid=' . $catId . '"> <img src="img/card-'.$catId. '.jpg" class="card-img-top" alt="image for this book" width="249px" height="350px">
                        <div class="card-body"> </a>
                            <h5 class="card-title"><a href="viewbookList.php?catid=' . $catId . '">' . $catname . '</a></h5>
                            <p class="card-text">' . substr($catdesc, 0, 29). '...</p>
                            <a href="viewbookList.php?catid=' . $catId . '" class="btn btn-primary">View All</a>
                        </div>
                    </div>
                </div>';
            }
    ...
    <div class="container my-3" id="cont">
        ...
        <?php 
            $query = $_GET["search"];
            $sql = "SELECT * FROM `book` WHERE MATCH(bookName, bookDesc) against('$query')"; 
            $result = mysqli_query($conn, $sql);
            while($row = mysqli_fetch_assoc($result)){
                ...
                echo '<div class="col-xs-3 col-sm-3 col-md-3">
                    <div class="card" style="width: 15rem;">
                    <a href="viewbook.php?bookId=' . $bookId . '"> <img src="img/book-'.$bookId. '.jpg" class="card-img-top" alt="image for this book" width="249px" height="350px">
                        <div class="card-body"></a>
                            <h5 class="card-title">' . substr($bookName, 0, 20). '...</h5>
                            <p>By ' . substr($writer,0,30). '</p>
                            <h6 style="color: #ff0000">Rs. '.$bookPrice. '/-</h6>
                            <p class="card-text">' . substr($bookDesc, 0, 29). '...</p>
                            <div class="row justify-content-center">';
                                if($loggedin){
                                    $quaSql = "SELECT `itemQuantity` FROM `viewcart` WHERE bookId = '$bookId' AND `userId`='$userId'";
                                    $quaresult = mysqli_query($conn, $quaSql);
                                    $quaExistRows = mysqli_num_rows($quaresult);
                                    if($quaExistRows == 0) {
                                        echo '<form action="partials/_manageCart.php" method="POST">
                                              <input type="hidden" name="itemId" value="'.$bookId. '">
